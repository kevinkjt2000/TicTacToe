...
bool TicTacToe::SetValue(int rowIndex, int colIndex, int playerIndex) {
  ...
  // at this point it is a legal move, so depending on the player index a marker
  // becomes placed
  if (playerIndex == 1) {
...
// returns 0 representing a tie, 1 representing a win by Player1, 2 representing
// a win by Player2, and 3 otherwise.
...
std::ostream& operator<<(std::ostream& out, const TicTacToe& x) {
  out << "\n " << x.board[0][0] << " | " << x.board[0][1] << " | "
      << x.board[0][2] << " \n"
      << "-----------\n " << x.board[1][0] << " | " << x.board[1][1] << " | "
      << x.board[1][2] << " \n"
      << "-----------\n " << x.board[2][0] << " | " << x.board[2][1] << " | "
      << x.board[2][2] << "\n\n";
  return out;
}
