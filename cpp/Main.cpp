...
SOCKET start_server(char* IP, char* port) {
  ...
  server_addr.sin_addr.s_addr =
      htonl(INADDR_ANY); // accept any incomping connection address
...
int main(int argc, char** argv) {
  ...
  std::cout
      << "Welcome to Network Tic-Tac-Toe\n------------------------------\n\n";
...
// Attempt connecting to the server passed as argv[1] on port argv[2]
  ...
  do {
    ...
    while (game.GetStatus() == 3) // status code 3 means no winner or tie, which
                                  // implies the game is still going
    {
      ...
      if (turn % 2 == 0) {
        if (isServer) {
          ...
          while ((row < 1 || row > 3 || col < 1 || col > 3) ||
            ...
            std::cout << "Enter row <space> column (both have to be 1-3 and "
                         "the location must be empty): ";
            std::cin >> row >> col;
          }
          char msg[] = {static_cast<char>(row + '0'), ' ',
                        static_cast<char>(col + '0'), '\0'};
          send_msg(clientSock, msg);
        } else {
    ...
    if (isServer) {
      ...
      if (goAgain) {
        ...
        std::cout << "Waiting on other player to decide whether they want to "
                     "go again or not...\n";
        char* msg = get_msg(clientSock);
      ...
      std::cout << "Waiting on other player to decide whether they want to go "
                   "again or not...\n";
      char* msg = get_msg(serverSock);
      goAgain = (strcmp(msg, "YES") == 0);
      delete msg;
